//验证码对象
const VCode = {
    chars: "23456789abcdefghjkmnpqrstABCDEFGHJKMNPQRST",//验证码中出现的字符
    Vlen: 4, //验证码长度
...
const Verifying = {
    ...
    checkRequired(inputArr) {
        let result = true; //创建一个用于记录验证结果的变量，默认为 true

        for (let input of inputArr) {
            if (input.value.trim() === '') {
                this.showError(input, "该项不能为空!"); //错误
                result = false;
            } else {
                this.showSuccess(input); //正确
            }
        }//for
